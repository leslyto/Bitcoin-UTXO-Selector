 ...
 * AND   a purchase amount of 6
 * WHEN  request to /api/prepare-unspent-outputs is made
 * THEN  outputs with value 6 should be returned */
it('uses strategy 0 if possible', async () => {
  const response = await request(app)
    .get('/api/prepare-unspent-outputs')
    .query({ address, amount: 6 })
    .send();
...
/* GIVEN a list of outputs with values 3, 8, 6, 9, 1
 * AND   a purchase amount of 11
 * WHEN  request to /api/prepare-unspent-outputs is made
 * THEN  outputs with transactions that sum the value 11 should be returned */
it('uses strategy 0 if possible', async () => {
  const response = await request(app)
    .get('/api/prepare-unspent-outputs')
    .query({ address, amount: 11 })
    .send();
